...
describe('Deck Of Cards API', () => {
    ...
    describe('deck', () => {
        ...
        it('should get multiple shuffled decks', async () => {
            const
                options = { deck_count: 4, shuffle: true },
                deck = await api.deck(options)

            expect(deck).to.have.property('shuffled').and.to.equal(options.shuffle)
            expect(deck).to.have.property('deck_id')
            expect(deck).to.have.property('success').and.to.equal(true)
            expect(deck).to.have.property('remaining').and.to.equal(DECK_SIZE * options.deck_count)
        })

        it('should get a partial deck', async () => {
            const
                options = { cards: ['AS', 'AH', 'AD', 'AC'], shuffle: true },
                deck = await api.deck(options)
    ...
    describe('reshuffle', () => {
        it('should reshuffle a deck', async () => {
            const
                deck = await api.deck(),
                reshuffled = await api.reshuffle(deck.deck_id)
    ...
    describe('draw', () => {
        it('should draw n number of cards', async () => {
            const
                deck = await api.deck({ shuffle: true }),
                options = { count: 2 }
            ...
            expect(drawn).to.have.property('remaining').and.to.equal(DECK_SIZE - options.count)
    ...
    describe('pile', () => {
        ...
        it('add | should add n number of cards to a pile', async () => {
            const
                options = { cards },
                deck = await api.pile(original.deck_id, pileName).add(options)

            expect(deck).to.have.property('deck_id').and.to.equal(original.deck_id)
            expect(deck).to.have.property('success').and.to.equal(true)
            expect(deck).to.have.property('remaining').and.to.equal(DECK_SIZE - cards.length)
        ...
        it('draw | should draw from the bottom', async () => {
            const
                options = { bottom: true },
                deck = await api.pile(original.deck_id, pileName).draw(options)
